...
def main():
    data_train = pd.read_csv('./ensemble/datasets/singlecoil_knee_train.csv')
    data_test = pd.read_csv('./ensemble/datasets/singlecoil_knee_test.csv')
    data_val = pd.read_csv('./ensemble/datasets/singlecoil_knee_val.csv')
    data_fastmriplus = pd.read_csv('./external/fastmri-plus/Annotations/knee.csv')
    ...
    for case in cases:
        data_fastmriplus_mod = data_fastmriplus.copy()
        data_fastmriplus_mod['file'] = data_fastmriplus['file'].apply(lambda x: 'knee/data/singlecoil_' + case + '/' + x + '.h5')
        if case == 'train':
            data_pack = data_train
        elif case == 'val':
            data_pack = data_val
        elif case == 'test':
            data_pack = data_test

        data_all_train = data_pack.merge(data_fastmriplus_mod.drop_duplicates(subset=['file']), left_on='filename', right_on='file', how='left', indicator=True)
        ...
        data_hs_train.to_csv('./ensemble/datasets/singlecoil_knee_' + case + '_hs.csv', index=False)
        data_pat_train = data_all_train[data_all_train['_merge'] == 'both']
        data_pat_train.to_csv('./ensemble/datasets/singlecoil_knee_' + case + '_pat.csv', index=False)
    ...
    data_set.to_csv('./ensemble/datasets/singlecoil_knee_fastmriplus.csv', index=False)
